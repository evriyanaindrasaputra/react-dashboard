...
import {addCustomer} from '~/lib/service'
import { queryClient } from '~/providers/App'
import { Customer } from '~/types/customer'
import { toast } from 'react-toastify'

type AddCustomer = Pick<Customer, "name" | "address" | "country" | "phone_number" | "job_title">;
type FormAddCustomerProps ={
  setIsOpen : () => void
}

const FormAddCustomer: React.FC<FormAddCustomerProps> = ({setIsOpen}) => {
  const { register, formState: { errors }, handleSubmit } = useForm<AddCustomer>()
  const { mutate, isLoading} = useMutation(addCustomer, {
    onSuccess: () => {
      toast.success('Tambah Kustomer Berhasil')
      setIsOpen()
      queryClient.invalidateQueries(['customer'])
    }
  })
  async function onSubmit(values: AddCustomer) {
    mutate(values)
  }
  return (
